...
Cpec <- pec::cindex(list("Cox X1+X2"=fit12,"Cox X1"=fit1,"Cox X2"=fit2),formula=Surv(time,status)~1,data=dat,eval.times=Inf,pred.times=10)
...
C3 <- pec::cindex(list("x"=dummy$x),formula=Surv(time,status)~1,data=dummy,eval.times=Inf,tiedPredictionsIn=TRUE)
C3a <- pec::cindex(list("x"=dummy$x),formula=Surv(time,status)~1,data=dummy,eval.times=Inf,tiedPredictionsIn=FALSE)
...
C4a <- pec::cindex(list("x"=dummy2$x),formula=Surv(time,status)~1,data=dummy2,eval.times=Inf,tiedPredictionsIn=FALSE,tiedOutcomeIn=TRUE)
C4b <- pec::cindex(list("x"=dummy2$x),formula=Surv(time,status)~1,data=dummy2,eval.times=Inf,tiedPredictionsIn=FALSE,tiedOutcomeIn=FALSE)
...
C5a <- pec::cindex(list("x"=dummy3$x),formula=Surv(time,status)~1,data=dummy3,eval.times=Inf,tiedPredictionsIn=TRUE,tiedOutcomeIn=TRUE)
C5b <- pec::cindex(list("x"=dummy3$x),formula=Surv(time,status)~1,data=dummy3,eval.times=Inf,tiedPredictionsIn=TRUE,tiedOutcomeIn=FALSE)
# }}}
...
set.seed(17)
Cbc <- pec::cindex(list("Cox X1"=fit1,"Cox X2"=fit2),formula=Surv(time,status)~1,data=dat,eval.times=Inf,B=2,splitMethod="bootcv",keep.index=TRUE,keep.matrix=TRUE)
...
Cbc.1 <- pec::cindex(list("Cox X1"=fit1.1,"Cox X2"=fit2.1),formula=Surv(time,status)~1,data=val1,eval.times=Inf)
...
Cbc.2 <- pec::cindex(list("Cox X1"=fit1.2,"Cox X2"=fit2.2),formula=Surv(time,status)~1,data=val2,eval.times=Inf)
...
## fit2 <- coxph(Surv(time,status)~X2,data=dat)
## set.seed(17)
## Cbc <- pec::cindex(list("Cox X1"=fit1,"Cox X2"=fit2),formula=Surv(time,status)~1,data=dat,eval.times=Inf,B=10,splitMethod="bootcv",keep.index=TRUE,keep.matrix=TRUE)
